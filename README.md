...
| 聊天記錄 | `/dashboard/history` | 查看與用戶的完整對話（Bot回覆/小編回覆以不同顏色區分）、貼標籤、備註、全域關鍵字搜尋（含關鍵字高亮與跳轉至該則訊息） |
...
*文件最後更新：2026-05-13（新增聊天記錄全域搜尋與關鍵字高亮；修正跳轉定位改用 MessageLog.id 直接定位，解決 MySQL DATETIME 秒精度碰撞問題）*
