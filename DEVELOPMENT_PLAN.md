...
| **已完成** | 每日附件報表（/admin/report）：傳送者/檔案類型/使用者/檔案名稱，可依日期查詢並匯出 CSV | ✅ |
...
後台 `/admin/report` 頁面，欄位：**時間 / 傳送者 / 檔案類型 / 使用者 / 檔案名稱**。

- 傳送者：LINE 用戶 display_name
- 使用者：歸檔檔名 `-` 前的姓名（待人工確認類型顯示「待人工確認」紅字）
- 日期選擇器：可查任意歷史日期，預設今日（CST）
- 統計卡片：總筆數 + 各類型分計
- 匯出 CSV：UTF-8 BOM，Excel 直接開啟
...
*文件最後更新：2026-06-07（全自動歸檔 + 身分證驗證重構 + 大頭照改原始檔名歸檔 + NAS fstab + 每日附件報表）*
